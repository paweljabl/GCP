...
This is a dataflowp pipeline that demonstrates Python use of side inputs. The pipeline finds Java packages
...
#   bucket = opts.bucket
  ...
  get_java_query='SELECT content FROM [hadoop-328915.ab.git_content_java] {0}'.format(limit_records)
# where id = 'fc1e0e717fc353cc78994b455b1153b438f92769'
  argv = [
...
#   bucket = opts.bucket
  ...
  if runner == 'DirectRunner':
     limit_records='LIMIT 3000'
  get_java_query="SELECT content FROM [hadoop-328915.ab.git_content_java] WHERE id = 'fc1e0e717fc353cc78994b455b1153b438f92769' {0}".format(limit_records)
# where id = 'fc1e0e717fc353cc78994b455b1153b438f92769'
  argv = [
    '--project={0}'.format(project),
    '--job_name=javahelpjob',
  ...
  output_results = 'gs://{0}/javahelp/MyResults'.format(bucket)  
  p = beam.Pipeline(argv=argv)

 # find all lines that contain the searchTerm
  (p
      | 'ReadFromBQ' >> beam.io.Read(beam.io.BigQuerySource(project=project,query=get_java_query))
      | 'PackageUse' >> beam.FlatMap(lambda rowdict: packageUse(rowdict['content'], 'import'))
      | 'TotalUse' >> beam.CombinePerKey(sum)
      | 'write' >> beam.io.WriteToText(output_results)
  )
